...
function module.GetClosestPlayer(fov)
    ...
    for i,v in pairs(dwPlayers:GetPlayers()) do
        if v ~= nil and v.Character ~= nil and v.Character:FindFirstChild("Head") ~= nil and v.Character:FindFirstChild("Torso") ~= nil and v.Character:FindFirstChild("HumanoidRootPart") ~= nil and module.aimvisibilitycheck(v.Character:FindFirstChild(module.Aimbot.TargetPart).Position or v.Character:FindFirstChild("Head").Position ,{dwLocalPlayer.Character, v.Character}) and v ~= dwLocalPlayer and module.aimteamcheck(v) and v.Character:GetAttribute("Health") ~= 0 then
